...
class Trainer(object):
    ...
    def train_one_epoch(self, epoch):
        ...
        for batch_idx, (inputs, calibs, targets, info,img_instances, mask_instances) in enumerate(self.train_loader):
            inputs = inputs.to(self.device)
            calibs = calibs.to(self.device)
            mask_instances = mask_instances.to(self.device)
            ...
            mask_dict=None
            #ipdb.set_trace()
            detr_losses_dict = self.detr_loss(outputs, targets, img_instances, mask_instances,mask_dict)
