...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        self.instance_dir = 'data/KITTIDataset/training/instance_2'
    ...
    def get_instance_label(self, idx):
        instance_label_file = os.path.join(self.instance_dir, '%06d.png' % idx)
        assert os.path.exists(instance_label_file)
        return Image.open(instance_label_file)
    ...
    def __getitem__(self, item):
        ...
        if self.split != 'test':
            img_instance = self.get_instance_label(index)
        ...
        if self.data_augmentation:
            ...
            if np.random.random() < self.random_flip:
                ...
                if self.split != 'test':
                    img_instance = img_instance.transpose(Image.FLIP_LEFT_RIGHT)
        ...
        if self.split != 'test':
            img_instance = img_instance.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
        ...
        # image instance encoding
        if self.split != 'test':
            img_instance = np.array(img_instance).astype(np.int16)
        info = {'img_id': index,
                'img_size': img_size,
        ...
        if self.split == 'test':
            ...
            img_instance, mask_instance = {}, {}
            return img, calib.P2, img, info, img_instance, mask_instance
        ...
        mask_instance = np.zeros((self.max_objs), dtype=np.int32)
        ...
        instance =np.nonzero(mask_2d)
    
        for idx in range(instance[0].shape[0]):
            mask_instance[idx] = instance[0][idx]
        ...
        info = {'img_id': index,
                'img_size': img_size,
                'bbox_downsample_ratio': img_size / features_size}
        return inputs, calib.P2, targets, info, img_instance, mask_instance
