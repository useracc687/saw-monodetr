...
class MonoDETR(nn.Module):
    ...
    def __init__(self, backbone, depthaware_transformer, depth_predictor, num_classes, num_queries, num_feature_levels,
        ...
        self.weight_embed = MLP(hidden_dim, hidden_dim, 3, 2)  # weight depth
        self.geo_uncer = MLP(hidden_dim, hidden_dim, 1, 2)  # weight depth
        ...
        if with_box_refine:
            ...
            self.weight_embed = _get_clones(self.weight_embed, num_pred)
            self.geo_uncer = _get_clones(self.geo_uncer, num_pred)
            ...
            self.weight_embed = nn.ModuleList([self.weight_embed for _ in range(num_pred)])
            self.geo_uncer = nn.ModuleList([self.geo_uncer for _ in range(num_pred)])
    ...
    def forward(self, images, calibs, targets, img_sizes,  dn_args=None):
        ...
        for lvl in range(hs.shape[0]):
            ...
            geo_uncer = self.geo_uncer[lvl](hs[lvl])  # (bs,N,1)
            # depth_geo
            box2d_height_norm = outputs_coord[:, :, 4] + outputs_coord[:, :, 5]
            box2d_height = torch.clamp(box2d_height_norm * img_sizes[:, 1: 2], min=1.0)
            depth_geo = ((1. / (geo_uncer[:, :, 0].sigmoid() + 1e-6) - 1.)+size3d[:, :, 0]) / box2d_height * calibs[:, 0, 0].unsqueeze(1)

            # depth_reg
            depth_reg = self.depth_embed[lvl](hs[lvl])
            
            depth_weight = self.weight_embed[lvl](hs[lvl])  # (bs,N,3)
            ...
            depth_map = F.grid_sample( # get value of center
            ...
            # depth average + sigma
            depth_ave = torch.cat(\
                [(depth_weight[:,:,0:1]*(1. / (depth_reg[:, :, 0: 1].sigmoid() + 1e-6) - 1.) \
                                + depth_weight[:,:,1:2] * depth_geo.unsqueeze(-1) \
                                    + depth_weight[:,:,2:3] * depth_map) / depth_weight.sum(dim=2,keepdim = True) ,
                                    depth_reg[:, :, 1: 2]], -1)
...
class SetCriterion(nn.Module):
    ...
    def loss_depth_map(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        losses["loss_depth_map"] = self.ddn_loss(
            depth_map_logits, gt_boxes2d, num_gt_per_img, gt_center_depth,**kwargs)
        return losses
    ...
    def forward(self, outputs, targets,img_instances, mask_instances, mask_dict=None):
        ...
        for loss in self.losses:
            #ipdb.set_trace()
            losses.update(self.get_loss(loss, outputs, targets, indices, num_boxes, instances= img_instances, mask = mask_instances))
            ...
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    if loss == 'depth_map':
                        ...
                        continue
                    kwargs = {'instance' : img_instances,'mask' : mask_instances}
                    if loss == 'labels':
